...
//    }
    ...
    public void onBackPressed() {
        ...
        //todo: in send back from here.
        Bundle bundle = getIntent().getExtras();
        if (bundle != null) {
            boolean BackInMainApp = bundle.getBoolean("BackInMainApp");
            Log.e("BackInMainApp", String.valueOf(BackInMainApp));
            if (BackInMainApp) {
                Intent launchIntent = getPackageManager().getLaunchIntentForPackage("com.docscan.android");
    ...
    void onCardDetected(Bitmap detectedBitmap, DetectionInfo dInfo) {
        try {
            Log.e("Detect Information : ", dInfo.creditCard().cardNumber);
...
//        Log.e("CardNumber 1: ",dInfo.creditCard().cardNumber);
...
//        mUIBar.setVisibility(View.INVISIBLE);
        ...
        //todo: launching back to docscan
        Intent launchIntent = getPackageManager().getLaunchIntentForPackage("com.docscan.android");
        if (launchIntent != null) {
            launchIntent.putExtra("CardNumber", mDetectedCard.cardNumber); // default: false
            launchIntent.putExtra("CardType", mDetectedCard.getCardType().name); // default: false
            Log.e("Start Result : ", String.valueOf(mDetectedCard));
            if (mDetectedCard.isExpiryValid()) {
                String resultDisplayStr = mDetectedCard.expiryMonth + "/" + mDetectedCard.expiryYear + "\n";
                launchIntent.putExtra("CreditCardExpiryDate", resultDisplayStr);
            }
            ...
            launchIntent.putExtra("CreditCardImage", scaledCardBytes.toByteArray());

            startActivity(launchIntent);//null pointer check in case package name was not found
            finish();
        } else {
            //todo: also sending here
            new android.app.AlertDialog.Builder(CardIOActivity.this)
                    .setTitle("Hint")
                    .setMessage("For the credit card scan you have to download Document scanner Application from play store..")
                    .setPositiveButton("Install",
                            new DialogInterface.OnClickListener() {
                                public void onClick(DialogInterface dialog,
                                                    int id) {
                                    startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=" + "com.docscan.android")));
                                    dialog.dismiss();
                                }
...
//        }
    ...
    private void setPreviewLayout() {
        ...
        Toast.makeText(this, "Scan only credit card..", Toast.LENGTH_LONG).show();
        ...
        if (getIntent() != null) {
            ...
            if (color != 0) {
                ...
                mOverlay.setGuideColor(Color.parseColor("#91c3eb"));
            }
...
//        }
    ...
    private void setResultAndFinish(final int resultCode, final Intent data) {
        ...
        //todo: setting result and back to docscan
        Intent launchIntent = getPackageManager().getLaunchIntentForPackage("com.docscan.android");
        if (launchIntent != null) {
            CreditCard scanResult = data.getParcelableExtra(CardIOActivity.EXTRA_SCAN_RESULT);
            launchIntent.putExtra("CardNumber", scanResult.cardNumber); // default: false
            launchIntent.putExtra("CardType", scanResult.getCardType().name); // default: false
            Log.e("Start Result : ", scanResult.getCardType().name);
            ByteArrayOutputStream scaledCardBytes = new ByteArrayOutputStream();
            mOverlay.getBitmap().compress(Bitmap.CompressFormat.JPEG, 80, scaledCardBytes);
            launchIntent.putExtra("CreditCardImage", scaledCardBytes.toByteArray());
