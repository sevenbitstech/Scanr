 ...
 */
public class DrivingLicenceValidation_1 {
    // Licence variable
    ...
    public DrivingLicenceValidation_1(Context context) {
    ...
    public boolean licenceVerification(Bitmap detectbitmap) {
        ...
        cutLowerHalfImage(detectbitmap);

        if (bo_licence_number && bo_licence_date) {

            cutLicenceNameImage(detectbitmap);
            cutAddressPortionImage(detectbitmap);
            if (CommonScan.CARD_ISSUE_ADDRESS.length() > 4 && CommonScan.CARD_HOLDER_NAME.length() > 4) {
    ...
    private Bitmap cutLicenceNameImage(Bitmap origialBitmap) {
        ...
        Rect desRect = new Rect(70, origialBitmap.getHeight() / 2, origialBitmap.getWidth() - 150, origialBitmap.getHeight() / 2 + 85);
...
//                duplicateName_value(oldBitmap);
    ...
    public void cutUpperHalfImage(Bitmap origialBitmap) {
        Bitmap cutBitmap = Bitmap.createBitmap(origialBitmap.getWidth() - 150,
                origialBitmap.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(cutBitmap);
        Rect desRect = new Rect(70, 0, origialBitmap.getWidth() - 150, origialBitmap.getHeight() / 2);
        canvas.drawBitmap(origialBitmap, desRect, desRect, null);
    ...
    public void cutLowerHalfImage(Bitmap origialBitmap) {
        Bitmap cutBitmap = Bitmap.createBitmap(origialBitmap.getWidth() - 150,
                origialBitmap.getHeight(), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(cutBitmap);
        Rect desRect = new Rect(70, origialBitmap.getHeight() / 2, origialBitmap.getWidth() - 150, origialBitmap.getHeight());
        canvas.drawBitmap(origialBitmap, desRect, desRect, null);
    ...
    public void cutAddressPortionImage(Bitmap origialBitmap) {
        ...
        for (int i = 0; i < textBlocks.size(); i++) {
            ...
            for (int j = 0; j < textBlock.getComponents().size(); j++) {
                System.out.println("call methods");
                if (textBlock.getComponents().get(j).getValue().trim().toLowerCase().contains("sign") || textBlock.getComponents().get(j).getValue().trim().toLowerCase().equals("ess")
                        || textBlock.getComponents().get(j).getValue().contains(CommonScan.CARD_HOLDER_DOB)) {

                } else {
                    CommonScan.CARD_ISSUE_ADDRESS += " " + textBlock.getComponents().get(j).getValue();
                }
