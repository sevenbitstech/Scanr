...
//        MainActivity.magnifierView.onCornerUnfocus();
    ...
    public List<PointF> getPointsShape(ImageView imageView) {

        List<PointF> list = new ArrayList<>();
        list.add(viewPointToImagePoint(new PointF(pointer1.getX(), pointer1.getY()), imageView));
        list.add(viewPointToImagePoint(new PointF(pointer2.getX(), pointer2.getY()), imageView));
        list.add(viewPointToImagePoint(new PointF(pointer3.getX(), pointer3.getY()), imageView));
        list.add(viewPointToImagePoint(new PointF(pointer4.getX(), pointer4.getY()), imageView));
        return list;
...
//         return list;
    ...
    private PointF viewPointToImagePoint(PointF point, ImageView imageView) {
        Matrix matrix = new Matrix();
        imageView.getImageMatrix().invert(matrix);
        return mapPointToMatrixGet(point, matrix);
    }

    private PointF mapPointToMatrixGet(PointF point, Matrix matrix) {
//        float[] points = new float[] { point.x+60 , point.y +60};
    ...
    public void setPoints(Map<Integer, PointF> pointFMap, Matrix matrix) {


        if (pointFMap.size() == 4) {
            setPointsCoordinates(pointFMap, matrix);
        }
        polygonView.invalidate();
    }

    private void setPointsCoordinates(Map<Integer, PointF> pointFMap, Matrix matrix) {

        pointer1.setX(imagePointToViewPoint(pointFMap.get(0), matrix).x);
        pointer1.setY(imagePointToViewPoint(pointFMap.get(0), matrix).y);

        pointer2.setX(imagePointToViewPoint(pointFMap.get(1), matrix).x);
        pointer2.setY(imagePointToViewPoint(pointFMap.get(1), matrix).y);

        pointer3.setX(imagePointToViewPoint(pointFMap.get(2), matrix).x);
        pointer3.setY(imagePointToViewPoint(pointFMap.get(2), matrix).y);

        pointer4.setX(imagePointToViewPoint(pointFMap.get(3), matrix).x);
        pointer4.setY(imagePointToViewPoint(pointFMap.get(3), matrix).y);
     ...
     */
    private PointF imagePointToViewPoint(PointF imgPoint, Matrix matrix) {
...
//        float[] points = new float[] { point.x-20, point.y-20 };
        float[] points = new float[]{point.x, point.y};
        matrix.mapPoints(points);
        if (points.length > 1) {
            return new PointF(points[0], points[1]);
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        int radius = ScanUtils.dp2px(context, 5);
